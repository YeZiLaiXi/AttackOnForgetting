...
def initialize_bucket(key, secret, endpoint, bucket, retries=10):
    ...
    for retry in range(retries):
        try:
            ...
            print("OSS bucket [{}] initialized.".format(bucket))
            return data_bucket
        except:
            print("OSS bucket [{}] initialization failed. Retrying... {}".format(bucket, retry))
...
def read_from_buffer(bucket, oss_file, bucket_name, retries=10):
    ...
    for retry in range(retries):
        try:
            ...
            if retry < retries-1:
                print("OSS download failed. {}/{} File: {}. Retrying...".format(
                    retry+1, retries, oss_file, 
                ))
            else:
                print("OSS download failed. File: {}. Trying other videos.".format(
                    oss_file
                ))
...
def read_from_bucket(bucket, oss_file, local_file, bucket_name, retries=10):
    ...
    for i in range(retries):
        try:
            ...
            if i == retries-1:
                print('Exceed maxmium tries for getting file {}'.format(
                    oss_file
                ))
    return True
