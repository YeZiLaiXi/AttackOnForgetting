...
from .ViT_prompt import Prompt, PromptSplit
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if prompt_length is not None and pool_size is not None:
            if prompt_pool:
        ...
        if prompt_length is not None: 
            if self.prompt_type == "split":
                self.prompt = PromptSplit(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
                        prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size_old=pool_size[0], pool_size_new=pool_size[1], top_k=top_k, batchwise_prompt=batchwise_prompt,
    ...
    def forward_features(self, 
        ...
        if cap_layer != -1 and upd_layer != -1:
            raise Exception("can not activate cap_layer and upd_layer at the same time!")
            ...
            if cap_layer != -1 or upd_layer != -1:
                ...
                for l in range(self.depth):
                    ...
                    if cap_layer != -1 and l == cap_layer:
                        res['x'] = x.clone().detach()
                        return res

                    # update relevant feature
                    if upd_layer != -1 and l == upd_layer:
                        # update cls token
                        if upd_targt == 'cls':
                            x[:, 0, :], _, _ = KPM.update(x[:, 0, :], memory=memory, update_proto=True)
                        elif upd_targt == 'prompt':
                            x[:, 1:self.total_prompt_len+1, :], _, _ = KPM.update(x[:, 1:self.total_prompt_len+1, :], memory=memory, update_proto=True)  
                        # update patch
                        elif upd_targt == 'patch':
                            x[:, self.total_prompt_len+1:, :], _, _ = KPM.update(x[:, self.total_prompt_len+1:, :], memory=memory, update_proto=True) 
                        # udpate all
                        elif upd_targt == 'all':
                            x, _, _ = KPM.update(x, memory=memory, update_proto=True) 
                        # no update
                        else:
                            x = x
            else:
    ...
    def forward_head(self, res, pre_logits: bool = False):
        ...
        elif self.head_type == 'token+prompt' and self.class_token:
